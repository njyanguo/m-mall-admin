...
class Ctrl{
	...
	signUp(req, res, next) {
		...
		if (!username || !password) return res.tools.setJson(1, '用户名或密码错误3')
	...
	signIn(req, res, next) {
		...
		if (!username || !password) return res.tools.setJson(1, '用户名或密码错误2')	
		...
		.then(doc => {
			switch (doc) {
	            case 0:
	            	res.tools.setJson(1, '用户名或密码错误0')
	            	break
	            case 1:
	                res.tools.setJson(1, '用户名或密码错误1')
	                break
	            case 2:
